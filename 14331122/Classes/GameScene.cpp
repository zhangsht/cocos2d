...
void GameScene::addWidgets() {
	...
	// timer
	time = Label::createWithTTF(ttfConfig, "5");
...
void GameScene::preloadMusic()
{
	...
	audio->preloadBackgroundMusic("music/yeshi.m4a");
}

void GameScene::playBgm()
{
	SimpleAudioEngine::getInstance()->playBackgroundMusic("music/yeshi.m4a", true);
}
...
// set bonus text when player has hit the key
void GameScene::setBonus(STATE_T t)
{
	...
	addChild(bonus_text, 5);
	scheduleOnce(schedule_selector(GameScene::removeBonusText), 0.5f);*/
	bonus_text->setPosition(visibleSize.width / 2, visibleSize.height / 2);
	addChild(bonus_text, 2);
}

void GameScene::removeBonusText(float dt)
{
	bonus_text->removeFromParent();
}
...
void GameScene::exitScene(cocos2d::Ref* pSender) {
	stopAc();
	Sleep(10);
	Director::getInstance()->replaceScene(TransitionSlideInL::create(2.0, WelcomeScene::createScene()));
}
